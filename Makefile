...
NDK_PATH ?= $(HOME)/Android/Sdk/ndk/28.0.13014099
NDK_PATH ?= $(HOME)/Android/Sdk/ndk/27.0.12077973
NDK_PATH ?= $(HOME)/Android/Sdk/ndk/26.3.11579264
...
ifeq ($(wildcard $(CC)),)
    CC := clang
    CXX := clang++
    $(warning NDK not found, using system clang)
endif
...
CFLAGS := -fPIC -shared -O2 -Wall -Wextra -Werror=return-type \
